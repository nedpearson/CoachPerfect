...
let HTTP_PORT = parseInt(process.env.PORT, 10) || 3000;
const WS_PORT_OFFSET = 579; // WS port = HTTP port + 579 (e.g. 3000 → 3579)
...
// ═══════════════════════════════════════════════════
// SHARED STATE (available to both HTTP + WS)
// ═══════════════════════════════════════════════════
...
const httpServer = http.createServer((req, res) => {
  ...
  const wsPort = HTTP_PORT + WS_PORT_OFFSET;
...
function broadcast(msg, excludeDeviceId) {
  ...
  for (const [id, peerWs] of peers) {
    if (id !== excludeDeviceId && peerWs.readyState === 1) {
      peerWs.send(data);
    }
  }
}
...
// ═══════════════════════════════════════════════════
// START — with port conflict recovery
// ═══════════════════════════════════════════════════

function tryListenHTTP(port, maxRetries) {
  return new Promise((resolve, reject) => {
    let attempts = 0;

    function attempt(currentPort) {
      // Use named handlers so we can clean up properly between retries.
      // Node keeps the listen-callback as a 'listening' listener even on
      // failure, so we must manage both events manually.
      function onError(err) {
        ...
        if (err.code === 'EADDRINUSE') {
          ...
          if (attempts < maxRetries) {
            ...
            console.log(`[Start] Port ${currentPort} in use, trying ${nextPort}...`);
            attempt(nextPort);
          } else {
            reject(new Error(`All ports ${port}-${currentPort} are in use. Set PORT env to pick a free port.`));
          }
        } else {
          reject(err);
        }
...
async function start() {
  try {
    // 1. Start HTTP server (with port fallback)
    const actualHttpPort = await tryListenHTTP(HTTP_PORT, 10);
    HTTP_PORT = actualHttpPort;

    // 2. Start WebSocket server
    const wsPort = HTTP_PORT + WS_PORT_OFFSET;
    try {
      wss = await startWebSocketServer(wsPort);
    } catch (wsErr) {
      if (wsErr.code === 'EADDRINUSE') {
        console.error(`[Start] WebSocket port ${wsPort} in use — sync disabled. Kill the conflicting process or set WS_PORT_OFFSET.`);
        // Continue without WS — HTTP still works
      } else {
        throw wsErr;
      }
    }
    ...
    // 3. Print banner
    const wsStatus = wss ? `ws://${LOCAL_IP}:${wsPort}` : 'UNAVAILABLE (port conflict)';
    console.log('');
    console.log('  ┌─────────────────────────────────────────────────┐');
    console.log('  │                                                 │');
    ...
    console.log(`  │   Sync:    ${wsStatus}`);
    console.log('  │                                                 │');
    console.log('  │   👉 Open /nexus to launch the app              │');
    ...
    console.error('');
    console.error('  ❌  Coach Perfect failed to start:');
    console.error('  ' + err.message);
    console.error('');
    console.error('  Fix: PORT=3007 node server.js   (pick a free port)');
    console.error('');
    process.exit(1);
  }
}
